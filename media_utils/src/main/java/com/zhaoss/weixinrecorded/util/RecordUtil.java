...
public class RecordUtil {
    ...
    public RecordUtil(String videoPath, String audioPath, int videoWidth, int videoHeight, int rotation, boolean isFrontCamera){
        ...
        try {
            ...
            if(videoFile.exists()){
                boolean isVideoDelete = videoFile.delete();
            }
            boolean isCreate = videoFile.createNewFile();
            videoOut = new FileOutputStream(videoFile);

            File audioFile = new File(audioPath);
            if(audioFile.exists()){
                boolean isAudioFileDelete = audioFile.delete();
            }
            boolean isAudioFileCreate = audioFile.createNewFile();
            audioOut = new FileOutputStream(audioFile);
        } catch (Exception e) {
            Log.e("RecorderUtil",e.getMessage());
        }
    }
