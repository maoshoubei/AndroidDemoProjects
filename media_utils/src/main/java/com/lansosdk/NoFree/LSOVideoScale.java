...
public class LSOVideoScale {
    ...
    public boolean start() {
        ...
        if(mediaInfo.prepare()){
            ...
            if(mediaInfo.getvBitRate()>10*1024*1024){
                scaleWidth=mediaInfo.getWidth();
                scaleHeight=mediaInfo.getHeight();
            }else{  //做缩放处理.
                if(mediaInfo.getvWidth()* mediaInfo.getvHeight()==1280*720){
                ...
                }else if(mediaInfo.getvWidth() * mediaInfo.getvHeight()==1920*1080){
                    if(mediaInfo.getWidth()>mediaInfo.getHeight()){
                        scaleWidth=960;
                        scaleHeight=544;
                    }else {
                ...
                }else if(mediaInfo.getvWidth() * mediaInfo.getHeight()>1920*1080){
                    scaleWidth=mediaInfo.getWidth()/2;
                    scaleHeight=mediaInfo.getHeight()/2;
                }else if(mediaInfo.getvWidth() * mediaInfo.getvHeight()>1280*720){
                    scaleWidth=mediaInfo.getWidth()/2;
                    scaleHeight=mediaInfo.getHeight()/2;
                }
            }
            ...
            vScale.setScaleProgessListener(new onScaleProgressListener() {
                ...
                public void onProgress(ScaleExecute v, long currentTimeUS) {
                    if(progressListener!=null){
                        float time = (float) currentTimeUS / (float)(mediaInfo.getvDuration()*1000000);
