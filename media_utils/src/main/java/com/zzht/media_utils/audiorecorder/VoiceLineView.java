...
public class VoiceLineView extends View {
    ...
    private void drawMiddleLine(Canvas canvas) {
        ...
        canvas.save();
        float heightDivision2 = getHeight() / 2;

        canvas.drawRect(0, heightDivision2 - middleLineHeight / 2, getWidth(), heightDivision2 + middleLineHeight / 2, paint);
        canvas.restore();
    }
    ...
    private void drawVoiceLine(Canvas canvas) {
        ...
        for (int i = 0; i < paths.size(); i++) {
            paths.get(i).reset();
            paths.get(i).moveTo(getWidth(), heightDivision2);
        }
        float iFloat = (float) getWidth() - 1;
        for (float i = iFloat; i >= 0; i -= fineness) {
    ...
    private void drawVoiceRect(Canvas canvas) {
        ...
        double heightDivision2 = getHeight() / 2;
        if (speedY % totalWidth < 6) {
            Rect rect = new Rect((int) (-rectWidth - 10 - speedY + speedY % totalWidth),
                    (int) (heightDivision2 - rectInitHeight / 2 - (volume == 10 ? 0 : volume / 2)),
    ...
    public void setVolume(int volume) {
        if (volume > maxVolume * sensibility / 25) {
            isSet = true;
            int heightMultiplyVolume = getHeight() * volume;
            float result = heightMultiplyVolume / 2;
            this.targetVolume = result / maxVolume;
        }
    }
    ...
    private void lineChange() {
        ...
        if (volume < targetVolume && isSet) {
            volume += (float) (getHeight() / 30);
        } else {
            isSet = false;
            if (volume <= 10) {
                volume = 10;
            } else {
                if (volume < (float) (getHeight() / 30)) {
                    volume -= (float) (getHeight() / 60);
                } else {
                    volume -= (float) (getHeight() / 30);
                }
            }
